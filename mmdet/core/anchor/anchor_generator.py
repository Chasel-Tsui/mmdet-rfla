...
class AnchorGenerator:
    ...
    def anchor2rf(self, multi_level_anchors, decay=0.5, device='cuda'):
        # 1/sqrt(n), P2, P3, P4, P5, P6: 10.6, 19, 41.7, 60.4, ..., base 4
        multi_level_rf = []
        for num in range(len(multi_level_anchors)):
            bboxes = multi_level_anchors[num]
            if num == 0:
                # P2, 
                ...
                factor = 10.6 / 16
                bboxes[..., 0] = (center_x2 - w2*factor/2)
                bboxes[..., 1] = (center_y2 - h2*factor/2)
                bboxes[..., 2] = (center_x2 + w2*factor/2)
                bboxes[..., 3] = (center_y2 + h2*factor/2)
                new_w2 = bboxes[..., 2] - bboxes[..., 0]
                new_h2 = bboxes[..., 3] - bboxes[..., 1]
                new_scale = new_h2 * new_w2
                #print(new_scale)
            if num == 1:
                w2 = bboxes[..., 2] - bboxes[..., 0]
                h2 = bboxes[..., 3] - bboxes[..., 1] 
            ...
            if num == 3:
                ...
                bboxes[..., 2] = (center_x2 + w2*factor/2)
                bboxes[..., 3] = (center_y2 + h2*factor/2)
            multi_level_rf.append(bboxes)


        return multi_level_rf
...
class RFGenerator:
    ...
    def gen_base_anchors(self):
        ...
        if self.fpn_layer == 'p3':
            self.base_sizes = [91, 267, 427, 555, 811]
        else:
            self.base_sizes = [45, 91, 267, 427, 555]
