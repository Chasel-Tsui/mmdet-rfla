...
class MLPRoIHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def _bbox_forward(self, x, rois, img_metas):
        ...
        avg_pool = torch.nn.AdaptiveAvgPool2d((1, 1)).cuda()
        max_pool = torch.nn.AdaptiveMaxPool2d((1, 1)).cuda()
        fc1 = torch.nn.Conv2d(in_channels=d1, out_channels=d1//ratio, kernel_size=1, stride=1, padding=0).cuda()
        relu1 = torch.nn.ReLU().cuda()
        fc2 = torch.nn.Conv2d(in_channels=d1//ratio, out_channels=d1, kernel_size=1, stride=1, padding=0).cuda()
        sigmoid = torch.nn.Sigmoid().cuda()
        # average pooling and max pooling
        avg_out = avg_pool(bbox_feats) #1024, 256, 1, 1
        avg_out = avg_out.permute(1, 0, 2, 3) #256, 1024, 1, 1
        avg_out = fc2(relu1(fc1(avg_out))) #256, 1024, 1, 1
 
        max_out = max_pool(bbox_feats) #1024, 256, 1, 1
        max_out = max_out.permute(1, 0, 2, 3) #256, 1024, 1, 1
        max_out = fc2(relu1(fc1(max_out))) #256, 1024, 1, 1
        # sum
        out = avg_out+max_out  # 256, 1024, 1, 1
        out_atten = sigmoid(out)
        out_atten = out_atten.permute(1, 0, 2, 3) 
        out_atten = out_atten.repeat(1,1,d4,d4)
        bbox_feats_output = out_atten * bbox_feats_ori
