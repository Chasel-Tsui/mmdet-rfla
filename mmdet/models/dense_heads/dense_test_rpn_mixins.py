...
class BBoxTestRPNMixin(object):
    ...
    def simple_test_rpn(self, x, img_metas):
        ...
        if len(rpn_outs) == 2:
            ...
            outs1 = (rpn_outs[0], rpn_outs[1])
            outs2 = (rpn_outs[2], rpn_outs[3])
            outs3 = (rpn_outs[4], rpn_outs[5])
            proposal_list1 = self.get_bboxes(*outs1, img_metas)
            proposal_list2 = self.get_bboxes(*outs2, img_metas)
            proposal_list3 = self.get_bboxes(*outs3, img_metas)
            batch = len(proposal_list1)
            proposal_list = []
            for i in range(batch):
                proposal1 = proposal_list1[i]
                proposal2 = proposal_list2[i]
                proposal3 = proposal_list3[i]
                proposal = torch.cat((proposal1, proposal2, proposal3), 0)
